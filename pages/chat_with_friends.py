...
if 'current_user' not in st.session_state or not st.session_state['current_user']:
    ...
    tag = "friend"
    chatter_name = st.text_input("Enter Friend's Name:")

    # Creating two columns for MND patient and the specific Friend
    col_family, col_mnd = st.columns(2)

    # Friend's message input
    with col_family:
        if chatter_name:
            st.subheader(f"Chat from {chatter_name}")
        family_message = st.text_area(f"Message from {chatter_name}:", key="friend_message")
        if st.button("Send as Friend", key="send_friend") and chatter_name:
